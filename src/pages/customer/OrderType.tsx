  ...
  return (
    <div className="max-w-4xl mx-auto py-8">
      <div className="text-center mb-12">
        <h1 className="font-serif font-black text-4xl sm:text-5xl text-white mb-4">How would you like to dine?</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        ...
        <button
          ...
          className={`flex flex-col items-center gap-4 p-8 rounded-3xl border transition-all ${
            orderType === 'Dine In'
              ? 'bg-brand-maroon-800 border-brand-orange text-white shadow-lg shadow-brand-orange/15 scale-[1.03]'
              : 'bg-card-dark/60 border-brand-maroon-800 text-brand-offwhite/70 hover:border-brand-maroon-700'
          }`}
        >
          <div className={`p-4 rounded-full ${orderType === 'Dine In' ? 'bg-brand-orange text-white' : 'bg-brand-maroon-900/60 text-brand-gold'}`}>
            <Utensils className="w-8 h-8" />
          </div>
          <span className="font-serif text-2xl font-bold">Dine In</span>
        ...
        >
          <div className={`p-4 rounded-full ${orderType === 'Take Away' ? 'bg-brand-orange text-white' : 'bg-brand-maroon-900/60 text-brand-gold'}`}>
            <ShoppingBag className="w-8 h-8" />
          </div>
          <span className="font-serif text-2xl font-bold">Take Away</span>
          <p className="text-sm text-center opacity-80 leading-relaxed">
        ...
        >
          <div className={`p-4 rounded-full ${orderType === 'Delivery' ? 'bg-brand-orange text-white' : 'bg-brand-maroon-900/60 text-brand-gold'}`}>
            <Truck className="w-8 h-8" />
          </div>
          <span className="font-serif text-2xl font-bold">Delivery</span>
          <p className="text-sm text-center opacity-80 leading-relaxed">
      ...
      <div className="max-w-md mx-auto">
        {orderType === 'Dine In' && (
          ...
          >
            <label className="block font-serif text-lg font-bold text-white mb-2 flex items-center gap-2">
            ...
            <input
              ...
              className="w-full bg-brand-maroon-900/60 border border-brand-maroon-800 rounded-2xl px-5 py-4 text-white text-lg focus:outline-none focus:border-brand-orange"
            />
          </motion.div>
        )}
        ...
        {orderType === 'Delivery' && (
          ...
          >
            ...
            <textarea
              ...
              className="w-full bg-brand-maroon-900/60 border border-brand-maroon-800 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-brand-orange resize-none"
            />
          </motion.div>
        )}

        {error && <p className="text-red-500 text-center text-sm font-bold mt-4">{error}</p>}
